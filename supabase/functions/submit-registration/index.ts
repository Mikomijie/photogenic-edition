...
serve(async (req) => {
  ...
  try {
    ...
    const tokenLooksValid = !!AIRTABLE_TOKEN && AIRTABLE_TOKEN.length > 40 && AIRTABLE_TOKEN.toLowerCase().startsWith('pat');

    if (!AIRTABLE_TOKEN || !tokenLooksValid) {
      return new Response(
        JSON.stringify({ success: false, error: 'Server misconfigured: invalid Airtable token' }),
    ...
    console.log('Received registration for:', payload.email);
    ...
    // Build Airtable payload — files already uploaded to storage by client
    ...
    if (payload.photoUrl) {
      airtableFields["Professional Photo"] = [{ url: payload.photoUrl }];
    }
    if (payload.videoUrl) {
      airtableFields["Introduction Video"] = [{ url: payload.videoUrl }];
    }

    console.log('Submitting to Airtable, photo:', !!payload.photoUrl, 'video:', !!payload.videoUrl);
    ...
    if (!airtableResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to submit to Airtable', details: airtableResult }),
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'Registration submitted successfully', emailSent }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
